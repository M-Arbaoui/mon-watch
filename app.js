...
/* ── Routing ── */
function goTo(name,skipSave=false){
...
function renderHero(item){
  ...
  document.getElementById('hero-bg').style.backgroundImage=bg?`url(${bg})`:'linear-gradient(135deg,#141416,#09090b)';
...
function startHeroTimer(){clearInterval(S.heroTimer);S.heroTimer=setInterval(()=>{S.heroIdx=(S.heroIdx+1)%S.heroItems.length;renderHero(S.heroItems[S.heroIdx]);},7000);}
function renderSpotlight(item){
  if(!item)return;
  const bg=imgB(item.backdrop_path);
  if(bg)document.getElementById('spotlight-bg').style.backgroundImage=`url(${bg})`;
  document.getElementById('spotlight-title').textContent=ttl(item);
  document.getElementById('spotlight-quote').textContent=(item.overview||'').slice(0,110)+'…';
  document.getElementById('spotlight').onclick=()=>openTitlePage(item);
}
...
async function loadHome(){
  ...
  if(trending.length){setupHero(trending);renderSpotlight(trending[Math.floor(Math.random()*Math.min(trending.length,8))]);}
  refreshContRow();
...
async function initDiscover(){
  ...
  const items=S.discoverType==='movie'?await A.discoverM(page):await A.discoverTV(page);
  S.discoverItems=items.filter(i=>i.backdrop_path);
  S.discoverIdx=0;
...
function triggerAutoNext(){
  ...
  if(cur===-1||cur===S.playerEps.length-1){showToast('End of season');return;}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.getElementById('player-back-btn').addEventListener('click',()=>{stopPlayer();goTo(S.lastPage||'home');});
  ...
  // Title page back
  document.getElementById('tp-back-btn')?.addEventListener('click',()=>goTo(S.titlePrevPage||'home'));

  // Person page back
  document.getElementById('person-back-btn')?.addEventListener('click',()=>{
    if(S.titleItem)openTitlePage(S.titleItem);else goTo(S.lastPage||'home');
  });
  ...
  if('serviceWorker' in navigator){
    navigator.serviceWorker.register('/watchy/sw.js',{scope:'/watchy/'})
...
const SERVERS = [
  ...
  { id:'vidsrc2',  label:'VidSrc 2', movie:id=>`https://vidsrc.xyz/embed/movie?tmdb=${id}`,  tv:(id,s,e)=>`https://vidsrc.xyz/embed/tv?tmdb=${id}&season=${s}&episode=${e}` },
  { id:'embed2',   label:'2Embed',   movie:id=>`https://www.2embed.cc/embed/${id}`,           tv:(id,s,e)=>`https://www.2embed.cc/embedtv/${id}&s=${s}&e=${e}` },
...
function updateServerUI(){
  document.querySelectorAll('.server-btn').forEach((b,i)=>b.classList.toggle('active',i===_currentServer));
...
function buildServerSwitcher(){
  ...
  SERVERS.forEach((srv, i)=>{
    const btn = document.createElement('button');
    btn.className = `server-btn${i===_currentServer?' active':''}`;
    btn.textContent = srv.label;
    btn.addEventListener('click', ()=>setServer(i));
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  setTimeout(()=>{
    ...
    document.getElementById('server-panel-toggle')?.addEventListener('click',()=>{
      document.getElementById('server-panel')?.classList.toggle('open');
    });
...
async function shareCard(item){
  ...
  ctx.textAlign = 'left';
  ctx.fillText('m-arbaoui.github.io/watchy', 52, H - 44);
  ctx.textAlign = 'right';
  ...
  canvas.toBlob(async blob => {
    ...
    if(navigator.canShare?.({files:[file]})){
      try{
        const movieUrl = `https://m-arbaoui.github.io/watchy/#/title/${type}/${item.id}`;
        await navigator.share({
          files:[file],
          title:`${title} on Watchy.`,
          text:`${title} (${year}) — Watch it on Watchy.`,
          url: movieUrl
        });
        return;
      }catch(_){}
    ...
    showToast('Card saved · Link copied');
  }, 'image/png');
}
